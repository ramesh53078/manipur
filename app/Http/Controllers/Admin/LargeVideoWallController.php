...
use DataTables;
class LargeVideoWallController extends Controller
{
    public function list(Request $request)
    {
        if ($request->ajax()) {
            $data = LargeVideoWall::get();
            ...
            ->addColumn('action', function ($row) {
                $editRoute = route('admin.largevideowall.edit', ['id' => $row->id]);
                $deleteRoute = route('admin.largevideowall.delete', ['id' => $row->id]);
        ...
        return view('admin.largevideowall.list');
    }

    public function create()
    {
        return view('admin.largevideowall.createOrUpdate');
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('video_thumbnail') && $request->hasFile('video')) {
                ...
                return redirect()->route('admin.largevideowall.list')->with('success', 'Large Video Wall updated successfully');
                ...
                // Log the error
                Log::error('Error creating LargeVideoWall: ' . $e->getMessage());
        
                // Redirect back with an error message
                return redirect()->back()->with('error', 'An error occurred while creating the LargeVideoWall. Please try again.');
            }
    }

    public function edit($id)
    {
        $timeWall = LargeVideoWall::findOrFail($id);
        $mode = 'edit';

        return view('admin.largevideowall.createOrUpdate', compact('timeWall','mode'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            // Log the error
            Log::error('Error updating Large Video Wall: ' . $e->getMessage());

            // Redirect back with an error message
            return redirect()->back()->with('error', 'An error occurred while updating the Large Video Wall. Please try again.');
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('admin.largevideowall.list')->with('success', 'Large Video Wall deleted successfully');
        } catch (\Exception $e) {
            DB::rollBack(); // Roll back the transaction in case of an exception

            // Log the error
            Log::error('Error deleting Large Video Wall: ' . $e->getMessage());

            // Redirect back with an error message
            return redirect()->route('admin.largevideowall.list')->with('error', 'An error occurred while deleting the Large Video Wall. Please try again.');
        }
    }
